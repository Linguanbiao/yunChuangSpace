...
<script>
import { getMessageApi, deletaMessageApi } from "@/api/message.js";
...
export default {
  ...
  methods: {
    fetchMessageData() {
      getMessageApi(this.curPage, this.eachPage).then((res) => {
    ...
    deleteMessage(messageInfo) {
      ...
      this.$confirm("此操作将永久删除该评论, 是否继续?", "是否删除该评论？", {
        ...
        .then(() => {
          // 确定删除分支
          deletaMessageApi(messageInfo).then((res) => {
            this.fetchMessageData(); // 重新渲染一次
            this.$message({
              type: "success",
              message: "删除成功!",
            });
